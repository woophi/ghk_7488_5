...
export const App = () => {
  ...
  useEffect(() => {
    ...
    window.gtag('event', '7488_selection_impression', { var: 'var5' });
  }, []);
  useEffect(() => {
    if (view === 'final') {
      window.gtag('event', '7488_quiz_impression', { var: 'var5' });
    }
  }, [view]);
  ...
  const submit = () => {
    window.gtag('event', '7488_continue_click', { var: 'var5', answer: finalAnswer });
      ...
      <>
        <div className={appSt.container}>
          ...
          {answers.map(dataAnswer => (
            <PureCell
              onClick={() => {
                setFinalAnswer(dataAnswer.answer(100));
              }}
              className={appSt.cellAnswer}
            >
              <PureCell.Content>
                <PureCell.Main>
                  <Typography.Text view="primary-medium">{dataAnswer.answer(100)}</Typography.Text>
                </PureCell.Main>
              </PureCell.Content>
              <PureCell.Addon verticalAlign="center">
                <Radio
                  checked={finalAnswer === dataAnswer.answer(100)}
                  onChange={() => setFinalAnswer(dataAnswer.answer(100))}
                />
              </PureCell.Addon>
            </PureCell>
          ))}
